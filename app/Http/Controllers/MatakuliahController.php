...
class MatakuliahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $query->where('kdmatkul2010054', 'LIKE', "%{$search}%")
                ->orWhere('namamat2010054', 'LIKE', "%{$search}%");
        }
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            'kdmatkul2010054' => 'required|unique:matakuliah2010054,kdmatkul2010054|min:7|max:7',
            'namamat2010054' => 'required|max:100',
            'sks2010054' => 'required|max:100',
        ], [
            'kdmatkul2010054.required' => ':atribut tidak boleh kosong',
            'kdmatkul2010054.unique' => ':atribut sudah ada',
            'kdmatkul2010054.min' => ':Minimal 7 Karakter',
            'kdmatkul2010054.max' => ':Maximal 7 Karakter',
            'namamat2010054.required' => ':atribut tidak boleh kosong',
            'sks2010054.required' => ':atribut tidak boleh kosong',
        ], [
            'kdmatkul2010054' => 'Kode Matakuliah',
            'namamat2010054' => 'Matakuliah',
            'sks2010054' => 'SKS',
        ]);
     ...
     */
    public function show(string $kdmatkul2010054)
    {
        $matakuliah = Matakuliah::find($kdmatkul2010054);
        if (!$matakuliah) {
     ...
     */
    public function update(Request $request, string $kdmatkul2010054)
    {
        $matakuliah = Matakuliah::find($kdmatkul2010054);
        if (!$matakuliah) {
            return response()->json(['message' => 'Data not found'], 404);
     ...
     */
    public function destroy($kdmatkul2010054)
    {
        $matakuliah = Matakuliah::find($kdmatkul2010054);
    ...
    public function uploadImage(Request $request, $kdmatkul2010054)
    {
        $matakuliah = Matakuliah::find($kdmatkul2010054);
        if (!$matakuliah) {
